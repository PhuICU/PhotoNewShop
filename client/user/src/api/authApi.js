...
export const checkOtp = async (data) => {
  try {
    const response = await api.post("/users/check-otp", data);
    return response.data;
  } catch (error) {
    return error;
  }
...
export const lockAccount = async (id) => api.put(`/users/lock-account`);

export const unlockAccount = async (id) => api.put(`/users/unlock-account`);

export const getUsers = async () => api.get("/users/all");
